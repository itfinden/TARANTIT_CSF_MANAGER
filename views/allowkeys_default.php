	...
	public function cancel() {
		...
		if ($key_details) {
			if (!$key_details['key_cancelled'] && $key_details['key_clicks_remained'] && $key_details['key_expire'] > time()) {
				...
				if ($key_details['key_cancelled']) {
					$errors[] = $instance->lang('keyalreadycancelled');
				}

				if (!$key_details['key_clicks_remained']) {
					$errors[] = $instance->lang('keyfullyused');
				}

				if ($key_details['key_expire'] <= time()) {
					$errors[] = $instance->lang('keyexpired');
				}
