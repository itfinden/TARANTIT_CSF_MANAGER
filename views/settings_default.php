	...
	public function save() {
		...
		if (is_array($config_values) && sizeof($config_values)) {
			foreach ($config_values as $config_name => $config_value) {
				...
				if ($config_name == 'allowlength') {
					$config_value = intval($config_value);
				}

				if ($config_name == 'allowlength_type') {
					$config_value = in_array($config_value, array('seconds', 'minutes', 'hours', 'days')) ? $config_value : 'days';
				}
