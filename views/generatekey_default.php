...
class jcsf_generatekey_default {
	...
	public function create() {
		...
		if ($output['data']['generate']['recipient'] && $output['data']['generate']['email'] && csfmanager::csfValidateEmail($output['data']['generate']['email']) && $client_id && isset($output['data']['clients'][$client_id]) && intval($output['data']['generate']['server']) && isset($output['data']['servers'][$output['data']['generate']['server']])) {
			...
			if (!$output['data']['generate']['recipient']) {
				$output['errormessages'][] = $instance->lang('emptyrecipientname');
			}

			if (!$output['data']['generate']['email']) {
				$output['errormessages'][] = $instance->lang('emptyrecipientemail');
			}

			if ($output['data']['generate']['email'] && !csfValidateEmail($generate['email'])) {
				$output['errormessages'][] = $instance->lang('invalidrecipientemail');
			}

			if (!$client_id) {
				$output['errormessages'][] = $instance->lang('emptyclient');
			}

			if ($client_id && !isset($output['data']['clients'][$client_id])) {
				$output['errormessages'][] = $instance->lang('invalidclient');
			}

			if (!intval($output['data']['generate']['server'])) {
				$output['errormessages'][] = $instance->lang('emptyserver');
			}

			if (intval($output['data']['generate']['server']) && !isset($output['data']['servers'][$output['data']['generate']['server']])) {
				$output['errormessages'][] = $instance->lang('invalidserver');
			}
