...
if (!defined('JCSF_ROOT_PATH')) {
	define('JCSF_ROOT_PATH', dirname(__FILE__));
}

if (!defined('WHMCS_ROOT_PATH')) {
	define('	', realpath(JCSF_ROOT_PATH . '/../'));
}
...
function itfinden_csf_manager_activate() {
	...
	sql_exec($sql, 'mod_csfmanager_config create');
	#sql_exec($sql);
	...
	sql_exec($sql, 'mod_csfmanager_config insert ');
	#sql_exec($sql);
	...
	sql_exec($sql, 'mod_csfmanager_allow create');
	#sql_exec($sql);
	...
	sql_exec($sql, 'mod_csfmanager_allow_keys create');
	#sql_exec($sql);
	...
	sql_exec($sql, 'tblemailtemplates insert ');
	#sql_exec($sql);
...
function itfinden_csf_manager_deactivate() {
	...
	sql_exec("DROP TABLE IF EXISTS `mod_csfmanager_allow_keys`");
	#sql_exec("DROP TABLE IF EXISTS `mod_csfmanager_allow`");
					...
					if (!$response['success']) {
						...
						if ($Firewall->setIP($ip)) {
							if (intval($instance->getConfig('allowlength')) > 0 && in_array($instance->getConfig('allowlength_type'), array('seconds', 'minutes', 'hours', 'days'))) {
								...
								if (intval($instance->getConfig('allowlength')) <= 0) {
									$output['errors'][] = $LANG['lengthinvalid'];
								}

								if (!in_array($instance->getConfig('allowlength_type'), array('seconds', 'minutes', 'hours', 'days'))) {
									$output['errors'][] = $LANG['lengthtypeinvalid'];
								}
						...
						case 'allow':
							...
							if ($submit) {
								...
								if ($ip && $reason && preg_match("/^[a-zA-Z\d\s]+$/", $reason)) {
									...
									if (!$reason) {
										$output['errors'][] = $LANG['noallowreason'];
									}

									if ($reason && !preg_match("/^\w\d\s$/", $reason)) {
										$output['errors'][] = $LANG['allowreasoninvalid'];
									}
						...
						case 'allowemail':
							...
							if ($submit) {
								...
								if ($fullname && $email && csfmanager::csfValidateEmail($email)) {
									...
									if (!$fullname) {
										$output['errors'][] = $LANG['emptyrecipientname'];
									}

									if (!$email) {
										$output['errors'][] = $LANG['emptyrecipientemail'];
									}

									if ($email && !csfValidateEmail($email)) {
										$output['errors'][] = $LANG['invalidrecipientemail'];
									}
