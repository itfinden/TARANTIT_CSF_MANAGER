...
class csfmanager {
	...
	static public function request_var($name, $default = null, $options = array()) {
		...
		if (isset($default)) {
			...
			case 'integer':return intval($value);
				break;
			case 'double':return floatval($value);
				break;
	...
	static public function csfPagination($start, $per_page, $num_items, $base_url) {
		...
		if ($prev_disabled) {
			$output .= "<li class=\"previous disabled\"><a href='#'>&laquo; " . $instance->lang('prevpage') . "</a></li>";
		} else {
			$output .= "<li class=\"previous\"><a href='{$base_url}{$url_delim}start=" . (($current_page - 2) * $per_page) . "'>&laquo; " . $instance->lang('prevpage') . "</a></li>";
		}

		if ($next_disabled) {
			$output .= "<li class=\"next disabled\"><a href='#'>" . $instance->lang('nextpage') . " &raquo;</a></li>";
		} else {
			$output .= "<li class=\"next\"><a href='{$base_url}{$url_delim}start=" . ($current_page * $per_page) . "'>" . $instance->lang('nextpage') . " &raquo;</a></li>";
		}
	...
	static public function sendCSFmail($messagename, $email, $fullname, $email_fields = array()) {
		...
		if (method_exists($whmcs, 'load_class')) {
